# Features
<!-- rev:008 (RFC 3339) 2026-08-24T00:00:00Z -->
...
| Compiled class/archive inventory | `modernlink inspect` | Reads `.class` headers plus validated direct-class `CONSTANT_Class` references, and indexes JAR/WAR/EAR entries for nested classfile major versions. It neither executes nor semantically decompiles bytecode; archive-entry reference traversal remains pending. |
...
| Canonical lifecycle plugin bundle | `cli/plugin/` | Includes harness-neutral analyze/architecture/domains/assess/plan/prepare/modernize/verify/migrate/status command contracts, evidence-first skills, specialist-role descriptors, and lifecycle rules; Codex/Claude materialization remains intentionally pending ownership contracts. |
