...
| SQL table-access inventory | `modernlink inspect` | Lexes repository `.sql` artifacts into cited `reads`/`writes` evidence and database-table nodes for `FROM`/`JOIN`, DML, and table DDL. It ignores quoted strings and SQL comments; it does not parse dialects, resolve dynamic SQL, inspect Java query literals, or infer table ownership. |
