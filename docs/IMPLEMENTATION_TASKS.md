...
<!-- rev:015 (RFC 3339) 2026-08-21T17:36:45Z -->
...
| ~~SC-04~~ | ~~Add `cargo fmt --check` + `cargo clippy -D warnings` jobs to CI~~ — **DONE `dd080b2`**; recorded command results are in [VERIFICATION.md](VERIFICATION.md) | `.github/workflows/test.yml` | — | S |
...
| ~~SC-08~~ | ~~Commit the ten previously untracked state docs~~ — **RECORDED AT ITS CHECKOFF REVISION**: `git ls-files` includes the named state docs. The current working tree is dirty for later work and does not reopen that historical item | repo root, `docs/` | — | S |
...
## Domain: verification (highest value — machine results are not a verdict)
...
| ~~VER-01~~ | ~~Stand up broker fixtures in CI for NATS, Kafka, Pulsar, RabbitMQ~~ — **RECORDED FOR THE FIXTURE SCOPE**: dedicated jobs invoke all five `#[ignore]`d tests; run 32386474212 recorded both broker-job conclusions at `3b64484`. Exact limits: [VERIFICATION.md](VERIFICATION.md) | `.github/workflows/test.yml` | — | L |
...
| ~~VER-07~~ | ~~Put `hacks/java6-messaging/` into a build path~~ — **DONE `76a17f0`**: the Dockerfile compiles the fixtures separately and the workflow invokes `LegacyJmsJmxDemo` | `docker/java6/Dockerfile`, `hacks/java6-messaging/` | — | S |
| ~~VER-02~~ | ~~Broker-backed send/receive/ack per provider~~ — **RECORDED FOR THE STATED HAPPY-PATH SCOPE**: all five targets exist and run 32386474212 recorded their dedicated jobs. Restart, reconnect, ordering under load, concurrency, failure, rollback/redelivery, and dead-letter behavior remain separate open work | `crates/messaging/tests/` | VER-01 | L |
| ~~VER-03~~ | ~~Run every Java test class in CI~~ — **DONE in the current workflow definition**: the 18 no-argument `*Test.class` files are discovered automatically, while `BrokerBackedMessagingTest` is invoked explicitly with a NATS endpoint. Local Docker invocations of all 19 sources exited 0; this workflow edit has not yet run in GitHub Actions | `.github/workflows/test.yml` | — | S |
...
| ~~VER-05~~ | ~~Native-load smoke test per platform~~ — **RECORDED FOR THE CONFIGURED SMOKE SCOPE**: Windows x86_64/JVM 21 and Linux x86_64/Java 6 have recorded runs; run 32386474212 recorded the linux-aarch64/JVM 21 load/assert steps. This is native-load evidence, not vendor-host compatibility | `java/src/test/java/com/modernlink/` | VER-03 | M |
