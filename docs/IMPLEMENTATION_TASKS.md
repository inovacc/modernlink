...
| VER-01 | Stand up broker fixtures in CI for NATS, Kafka, Pulsar, RabbitMQ — **partially done**: a `Broker-backed messaging` job starts `nats:2.10 -js` + `rabbitmq:3.13` via `docker run` (not `services:`, which cannot pass `-js`), waits on broker logs rather than ports, runs the three `#[ignore]`d tests and asserts all three ran. Kafka and Pulsar have their own job and their own tests. **All of it ran green on run 32386474212** — NATS, JetStream, RabbitMQ, Kafka and Pulsar against real brokers in CI. **VER-01 DONE** | `.github/workflows/test.yml` | — | L |
...
| ~~VER-03~~ | ~~Run all ten Java test classes in CI, not the current three~~ — **DONE `dd080b2`**: the workflow enumerates every class — **15** today — and the 13 that existed then ran green on runs 31781200582 and 31782837766 | `.github/workflows/test.yml` | — | S |
